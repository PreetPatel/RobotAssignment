...
public class TestCarrierRobot {
	...
	public void testBasicMovementAndPainting() {
		...
		Painter newpainter = new MockPainter();
		_topLevelNest.move(500, 500);
		_topLevelNest.paint(newpainter);
		assertEquals("(rectangle 4,4,100,100)(rectangle 4,4,50,50)(rectangle 9,9,10,10)(rectangle 3,3,5,5)", newpainter.toString());
	}
	...
	public void testAddWithOutOfBoundsArgument() {
		Robot robotToAdd = new WheeledRobot(80, 80, 2, 2, 50, 50);
		
		try {
			_topLevelNest.add(robotToAdd);
			fail();
		} catch(IllegalArgumentException e) {
			// Expected action. Ensure the state of topLevelNest and 
			// robotToAdd has not been changed.
			assertFalse(_topLevelNest.contains(robotToAdd));
			assertNull(robotToAdd.parent());
		}
	}
